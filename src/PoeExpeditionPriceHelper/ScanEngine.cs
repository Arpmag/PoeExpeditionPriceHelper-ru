...
internal sealed class ScanEngine : IDisposable
{
    ...
    private IReadOnlyList<PriceRow> BuildPriceRows(IReadOnlyList<OcrRow> ocrRows)
    {
        ...
        foreach (var row in ocrRows)
        {
            ...
            // Uncut gems — English and Russian client (poe2db: Неогранённый камень умения/духа/поддержки)
            if (TryResolveGemKey(row.NormalizedName, out var gemKey) ||
                RussianNameResolver.TryResolveUncutGem(
                    RussianNameResolver.PrepareForLookup(row.NormalizedName), out gemKey))
            {
            ...
            if (row.NormalizedName.Contains("random") && row.NormalizedName.Contains("currency") ||
                row.NormalizedName.Contains("случайн") && row.NormalizedName.Contains("валют"))
            {
            ...
            if (row.NormalizedName.Contains("unique") && row.NormalizedName.Contains("belt") ||
                row.NormalizedName.Contains("уникальн") && row.NormalizedName.Contains("пояс"))
            {
                rows.Add(new PriceRow(stableY, row.RawText, 0m, 0m, true, row.Multiplier, "unique belt", true, MemeKind.Headhunter));
                continue;
            }

            // Map Russian OCR → English poe.ninja key, then exact → prefix → fuzzy.
            var lookupName = RussianNameResolver.ToEnglishKey(row.NormalizedName) ?? row.NormalizedName;
            PriceEntry? entry;
            string matchedKey = lookupName;
            bool exact = false;
            if (snapshot.TryGetValue(lookupName, out entry))
            {
                exact = true;
            }
            else if (lookupName.Length >= 10 &&
                     snapshot.Keys.Where(k => k.StartsWith(lookupName, StringComparison.Ordinal))
                                  .MinBy(k => k.Length) is { } prefixKey)
            {
                entry = snapshot[prefixKey];
                matchedKey = prefixKey;
            }
            else if (lookupName.Length >= 6 &&
                     BestFuzzy(snapshot, lookupName) is { } fuzzy)
            {
