...
internal sealed class PriceOverlayForm : Form
{
    ...
    public void SetRegion(Rectangle regionRect, int xOffset)
    {
        if (IsDisposed) return;
        if (InvokeRequired) { BeginInvoke(() => SetRegion(regionRect, xOffset)); return; }
        _regionRect = regionRect;
        _xOffset = xOffset;
    ...
    private void DrawPrice(Graphics g, PriceRow row, int x, int screenY, bool highlightTop)
    {
        ...
        {
            DrawBackdrop(g, x, screenY, IconSize + 2 + TextWidth(g, "5 Зеркал Каландры"));
            ...
            g.DrawString("5 Зеркал Каландры", _priceFont, memeBrush, x + IconSize + 2, screenY - _priceFont.Height / 2);
            return;
        }
        if (row.Meme == MemeKind.Headhunter)
        {
            ...
            DrawBackdrop(g, x, screenY, hhWidth + 2 + TextWidth(g, "Охотник за головами!"));
            ...
            g.DrawString("Охотник за головами!", _priceFont, hhBrush, x + hhWidth + 2, screenY - _priceFont.Height / 2);
            return;
        }
        ...
        string eachWord = mult > 1 ? " за шт." : "";
        string label = mult > 1
            ? $"{total.ToString(fmt, inv)} ({unit.ToString(fmt, inv)}{eachWord})"
        ...
        lock (_lock)
        {
            if (_form is not null && !_form.IsDisposed)
            {
                ...
                existing.Invoke(() =>
                {
                    ...
                    {
                        existing.SetRegion(regionRect, xOffset);
