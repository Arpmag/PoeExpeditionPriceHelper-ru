...
internal sealed class OcrScanner
{
    ...
    private static readonly Regex MultiplierPattern = new(@"(?<![\p{L}0-9])(\d{1,3})\s*x(?![\p{L}0-9])", RegexOptions.Compiled);
    private static readonly Regex LeadingNoise = new(@"^(?:\S{1,2}\s+|\S*\d\S*\s+)+", RegexOptions.Compiled);
    private static readonly Regex QuantityMarker = new(@"(?<!\p{L})\d+\s*x\s+", RegexOptions.Compiled);
    ...
    {
        _engine = TryCreateRussianOcr()
            ?? OcrEngine.TryCreateFromUserProfileLanguages()
            ?? OcrEngine.TryCreateFromLanguage(new Language("en-US"))
            ?? throw new InvalidOperationException(
                "Windows OCR недоступен. Установите языковой пакет «Русский» в Параметры → Время и язык → Язык и регион → Русский → Параметры языка → OCR.");
        _log = log;
        _debug = debug;
    }
    ...
    private static OcrEngine? TryCreateRussianOcr()
    {
        foreach (var tag in new[] { "ru-RU", "ru" })
        {
            try
            {
