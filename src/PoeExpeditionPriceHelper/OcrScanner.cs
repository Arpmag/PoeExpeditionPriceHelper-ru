...
internal sealed class OcrScanner : IDisposable
{
    ...
    {
        // Russian game client — rus traineddata required in tessdata/
    ...
    internal static string StripLeadingNoise(string normalized)
    {
        ...
        // Strip leading non-letter chars (Latin or Cyrillic)
    ...
    internal static string NormalizeName(string text)
    {
        ...
        s = s.Replace('ё', 'е');
        // \p{L} keeps Cyrillic letters from the Russian client
        s = Regex.Replace(s, @"[^\p{L}\d\s]", " ");
        s = Regex.Replace(s, @"\s+", " ");
        return s.Trim();
    }
