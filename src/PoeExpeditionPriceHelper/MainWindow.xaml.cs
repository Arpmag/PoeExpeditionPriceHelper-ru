...
public partial class MainWindow : MetroWindow
{
    ...
    private void UpdateStatusLabel()
    {
        ...
        StatusLabel.Text = $"Загружено {_repo.ItemCount} предметов  ·  обновлено {fetched}";
    }

    private void DonateButton_Click(object sender, RoutedEventArgs e)
    {
        const string url = "https://www.paypal.com/donate/?business=pedro.levi.magic%40gmail.com&currency_code=USD&item_name=PoeExpeditionPriceHelper";
        try
        {
    ...
    internal void RunCalibration()
    {
        ...
        var wasRunning = _engine is not null;
        if (wasRunning)
        {
            _engine!.StopAndWait(TimeSpan.FromSeconds(2));
            _engine.Dispose();
    ...
    private void OnStateChanged(object? sender, EventArgs e)
    {
        ...
        {
            _trayIcon.ShowBalloonTip(3000, "Помощник цен Экспедиции",
                "Работает в фоне — дважды щёлкните по иконке в трее, чтобы открыть.",
    ...
    private void EnsureTrayIcon()
    {
        ...
        {
            Icon = icon,
            Text = "Помощник цен Экспедиции PoE 2",
            Visible = false,
        };
        ...
        switch (outcome)
        {
            ...
            case App.CaptureOutcome.Reserved:
                ...
                if (_rebindButton is not null)
                    _rebindButton.Content = $"{HotkeyBinding.Display(code)} занята — выберите другую";
                break;
    ...
    {
        if (_rebindButton is not null) _rebindButton.Content = "Назначить";
        _rebindButton = null;
        _rebindLabel = null;
